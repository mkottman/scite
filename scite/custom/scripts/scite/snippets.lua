...
--
-- Syntax:
--   Snippets are defined in a global table 'snippets'. Scopes and Lexers
--   (discussed below) are indices of the snippets table and are tables
--   themselves. Each string index in each of these tables is the snippet
--   trigger text. The expanded text is equated to this trigger.
--
--   For example:
--   snippets = {
--     file = "$(FileNameExt)",
...
--   }
--
--   The top-level snippets are global, the 'ruby' snippets are global to a
--   buffer with Ruby syntax highlighting enabled, and the ruby.string snippet
--   is expanded in that buffer only when currently in Ruby's string scope.
--
--   Scopes and Lexers:
...
local snippet_text, match_indention, join_lines, load_scopes, reset_keys
...
function insert(snippet_arg)
  ...
  if not snippet_arg then
    ...
    style    = editor.StyleAt[orig_pos]
    scope    = editor:GetStyleName(style)
  ...
  if s_text then
    ...
    -- Save key binding
    snippet.orig_tab  = _G.keys['\t']
    snippet.orig_stab = _G.keys['s\t']
    snippet.orig_esc  = _G.keys['esc']
    snippet.orig_ret  = _G.keys['\n']

    -- New temporary key binding
    _G.keys['\t']   = { next_snippet_item }
    _G.keys['s\t']  = { prev }
    _G.keys['esc']  = { cancel_current }
    _G.keys['\n']   = { function() while snippet.index do next_snippet_item() end end }
...
reset_keys = function()
  _G.keys['\t']   = snippet.orig_tab
  _G.keys['s\t']  = snippet.orig_stab
  _G.keys['esc']  = snippet.orig_esc
  _G.keys['\n']   = snippet.orig_ret
end
---
...
function list()
  ...
  if SCOPES_ENABLED then
    ...
    local scope = editor:GetStyleName(style)
...
function show_scope()
  ...
  local text = 'Lexer: '..lexer..'\nScope: '..
    editor:GetStyleName(scope)..' ('..scope..')'
