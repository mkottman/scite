...
-- adapted from http://lua-users.org/wiki/SciteAutoCompleteAnyLanguage
...
if ENABLE_AUTOCOMPLETE then
  ...
  function try_autocomplete()
      if not editor:AutoCActive() then
          ...
          for name, v in pairs(names) do
              if string.lower(string.sub(name, 1, len)) == string.lower(prefix) then
  ...
  -- these hooks are not used anywhere else
