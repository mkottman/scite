...
function update()
  ...
  if mlines[curr_line] then
    local s = editor:FindColumn( curr_line, mlines[curr_line].start_col )
    local e = editor:FindColumn(curr_line, curr_col)
    local delta = e - s
    local txt = ''
    if delta > 0 then txt = editor:textrange(s, e) end
    for line_num, item in pairs(mlines) do
      if line_num ~= curr_line then
        local next_pos = editor:FindColumn(line_num, item.start_col)
        if delta < 0 then
